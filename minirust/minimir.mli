...
type instr =
  ...
  | Iif of local * label * label
      (** branching to one of two instructions, depending on the value of a local, *)
  | Ireturn
      (** returning from the function, using the value of [Lref] as a return value, *)
  | Icall of string * local list * place * label
...
and rvalue =
  ...
  | RVconst of Ast_types.constant  (** a constant, *)
  | RVunit  (** the unit value, *)
  | RVborrow of mut * place  (** creating a new borrow of a place (mutable or shared), *)
  | RVbinop of Ast_types.binop * local * local  (** performing a binary operation, *)
  | RVunop of Ast_types.unop * local  (** performing a unary operation, *)
  | RVmake of string * local list  (** constructing a {v struct v} value.*)
...
type mir_body = {
  ...
  minstrs : (instr * Ast_types.location) array;
      (** CFG instructions, together with location information in the input file. *)
  mloc : Ast_types.location;  (** Location information for the whole function. *)
}
...
val typ_of_place : mir_body -> place -> full_typ
(** Compute the type of a place. *)

val place_mut : mir_body -> place -> mut
